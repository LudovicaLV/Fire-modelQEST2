...
import ModelFire2.SimulatorFire2;
import ParserProg5Fire.Parser5;
	...
	public static ArrayList<Integer> createListOneD (int x){
	 ...
	 //new code - for rules
		...
		public static boolean WhileMethod(){
			if(boundary == "#Time"){
				 if (SimulatorFire2.getTime() < timeTotal){return true;}
		         else{return false;}}
			else{
			if(boundary == "#Population"){
				if ((SumPopulation(nameAgentToTrack) > 0) && SimulatorFire2.getTime() < 1000 ){return true;}
		    	else{return false;}}
			else{
				if(boundary == "#Pop_loc"){
					if ((EntryPopLoc(nameAgentToTrack, LocToTrack) > 0) && SimulatorFire2.getTime() < 1000 ){return true;}
			    	else{return false;}}
			else{
				if ((valueForLoop < valueToCount) || SimulatorFire2.getTime() < 1000){return true;}
		    	else{return false;}    
				}}}}
