...
public class SimulatorFire2 {
	...
	public static void main(String[] args) {
		...
		int totalTime = TotalFire.TotalFire2.simulationTime;	
		outputFileLog = CreateModel2.WriteAllModelQEST2.Log();
		outputFileData = CreateModel2.WriteAllModelQEST2.Data();
		outputFileMeta = CreateModel2.WriteAllModelQEST2.Meta();
		outputFileCost = CreateModel2.WriteAllModelQEST2.Cost();
												...
												if (symbolPass == ".") {
													if (chosenActionPass.getUpdate().matches("(.*)new(.*)")) {
														...
														newPositionArrayP.set(action_index,locNewName);
															...
															ArrayList<ArrayList<Integer>> neighbourhood = GlobalManager
																	.getLocationManager().getNeighBouncing(PassAgentPosition, chosenActionPass.getRangePass());
															double[] ProbLoc = new double[neighbourhood.size()];
															for (int j = 0; j < neighbourhood.size(); j++) {
																ProbLoc[j] = (1.0 / neighbourhood.size());
															}
...
//		}
		...
		//HERE ALL THE PART IF THE POP ENDS< THEREFORE THE SIMULATION
		
//		int intTimeMore = totalTime - TimeArrayList.size();
//		
//		for (int i=intTime; i< intTimeMore; i++){
//			for (int j = 0; j < intAgents; j++){
//				for (int k=0; k< intLoc; k++){
//					ArrayList<Integer> Key = new ArrayList<>();
//					Key.add(i);
...
//			}
//		
//		for(int j=intTime; j < intTimeMore; j++){
//			timeArray[j] = TimeArrayList.get(j);
//		}
